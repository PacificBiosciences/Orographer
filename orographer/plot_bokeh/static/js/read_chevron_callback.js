...
function chevronVertices(x0Value, x1Value, yValue, targetLength, targetYOffset) {
    const span = Math.abs(x1Value - x0Value);
    if (span == 0) return { xs: [], ys: [] };
    const direction = Math.sign(x1Value - x0Value) == 1 ? 1 : -1;
    const tipOffset = span * tip_fraction;
    const length = Math.min(Math.max(1, targetLength), tipOffset);
